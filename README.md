...
韓国から日本就職を目指す求職者向けに、求人検索・意味的マッチングを支援するマルチエージェントプロジェクトです。

**CrewAI CLI** と **Streamlit MVP**（OpenAI 意味マッチ）が利用できます。**Vertex AI は Phase 2** で統合予定です。
...
- **差別化:** 非公開求人や年収交渉の代行ではなく、根拠付きマッチング（公開求人を信頼ソースとする）
...
| `OPENAI_EMBEDDING_MODEL` | 既定 `text-embedding-3-small`（最安 embedding 枠） |
| `OPENAI_CHAT_MODEL` | 既定 `gpt-5-nano`（最安 chat 枠） |
...
フロー: 職務履歴書テンプレート DL → 記入 → アップロード → **「이력서 분석」** → 都道府県選択 → **「매칭 실행」** → マッチング結果 + 選定求人の表示。
...
UI は 단계별 진행 상태(이력서 분석 → 공고 검색 → 의미 매칭 → URL 확인)를 표시합니다.  
매칭 이유 LLM 호출은 **상위 5건**만 생성합니다 (비용 절감).

### CLI

```bash
# Streamlit과 동일 R→E (기본)
uv run python main.py --prefecture 東京都

# 레거시 6단계 Crew 파이프라인 (이력서 최적화·면접 준비 포함)
uv run python main.py --mode full --prefecture 東京都
```

CLI MVP 모드는 `knowledge/resume.txt`를 사용합니다.
...
| 4 | Semantic match (E) | `matching_document` + JD blurb(상위 8건) + OpenAI 의미 매치 + URL 검증 |
| 5 | Job selection | 最適 1 件を選定（公開求人リンクを提示） |
...
UI 上でマッチング結果テーブルと選定求人（公開求人リンク）を表示します。
...
| Phase | 内容 |
|-------|------|
| **1（MVP）** | Streamlit、`ai_provider`（**OpenAI プライマリ**）、**R** resume intelligence、semantic match |
